...
class Character:
    """キャラクタークラス"""
    # ACTIONS = {0: "たたかう", 1: "ぼうぎょ"}
    # SubActions = {0: "単体攻撃", 1: "全体攻撃", 2: "単体回復",
    #               3: "炎属性呪文", 4: "氷属性呪文", 5: "光属性呪文", 6: "闇属性呪文",
    #               7: "ATKアップ", 8: "DEFアップ", 9: "AGIアップ", 10: "MagicATKアップ", 11: "MagicRECアップ",
    #               12: "ATKダウン", 13: "DEFダウン", 14: "AGIダウン", 15: "MagicATKダウン", 16: "MagicRECダウン",
    #               17: "耐性アップ", 18: "耐性ダウン", 19: "強攻撃", 20: "強全体攻撃", 21: "全体回復"}
    ...
    def action_select(self, playstyle, game):
        ...
        if playstyle == "RANDOM":
            ...
            if p < 0.85:
                ...
                #         self.target_enemy(self.SIDE, game), True)
                # elif self.Attribute_Actions[action] == "healing" or self.Attribute_Actions[action] == "bufsupport":
                #     target = self.select_mymember(
                #         self.myParty(self.SIDE, game), True)
        ...
        # playstyle == ATTACKING => 8割の確率でattackingを選択する残りの2割はattacking以外の中から選択
            ...
            #         self.target_enemy(self.SIDE, game), True)
            # elif self.Attribute_Actions[action] == "healing" or self.Attribute_Actions[action] == "bufsupport":
            #     target = self.select_mymember(self.myParty(self.SIDE, game), True)
        ...
        # 命大事に
            ...
            # # キャラクタの確認
            # for i in range(len(recovery_target)):
            #     c = recovery_target[i]
            #     print("回復対象 {}: {} \n".format(i, c.name))

            if len(recovery_target_id) > 0:
                # target = recovery_target_id[random.randrange(0, len(recovery_target_id), 1)]
    ...
    def action(self, game, action_flow):
        ...
        damages_enemy = 0
        recoveries_enemy = 0
        # action, sub_action, target_c = self.action_select(self.PlayStyle, game)
        ...
        # print(action, sub_action, target_c)
        # random行動の時
        # if auto == True:
        # selected_Action = random.randrange(0, len(self.ACTIONS), 1)
        ...
        # 単体攻撃
            ...
            # 対象キャラクタを選択する
            # target = self.select_target(
            #     self.target_enemy(self.SIDE, game), auto)
            # target = self.get_charaObj_by_id(target_id)
        ...
        # 回復魔法
            ...
            if self.MP < needMP:
                ...
                # 回復させるキャラクターを選択
                # mymember = self.select_mymember(
                #     self.myParty(self.SIDE, game), auto)
                mymember = target
                # 残りHPの計算
        ...
        # ATKダウン
            ...
            if self.MP < needMP:
                ...
                # 効くかどうかの判定
                # flag = 0  # 効果なし
                # if random.randrange(1, 101, 1) <= int(40*target.registance_rate(target.REGISTANCE["state"])):
                #     flag = 1
                flag = 1
        ...
        # 耐性ダウン (全体)
            ...
            if self.MP < needMP:
                ...
                # enemysリストの戦闘キャラクタのREGISTANCE["STATE"]を参照して、
                # そのキャラクタの耐性ダウン確率をもとに効くかどうかを決める

                # 効くかどうかの判定
                # flag = 0  # 効果なし
                # if random.randrange(1, 101, 1) <= int(40*enemys[0].registance_rate(enemys[0].REGISTANCE["state"])):
                #     flag = 1
